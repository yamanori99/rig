...
**latest** GitHub Release. After install it prints **one** line for the
current shell (`~/.zprofile` on macOS zsh, `~/.bashrc` on Linux bash)
so new terminals keep `rig` on `PATH`.
